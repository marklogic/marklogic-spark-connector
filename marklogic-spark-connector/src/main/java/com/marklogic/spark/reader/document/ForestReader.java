...
class ForestReader implements PartitionReader<InternalRow> {
    ...
    ForestReader(ForestPartition forestPartition, DocumentContext context) {
        ...
        this.callFactory = new ServerEvaluationCallFactory.Builder()
            .withInvokeOptionName(Options.READ_EXPAND_URIS_INVOKE)
            .withJavascriptOptionName(Options.READ_EXPAND_URIS_JAVASCRIPT)
            .withXqueryOptionName(Options.READ_EXPAND_URIS_XQUERY)
            .withJavascriptFileOptionName(Options.READ_EXPAND_URIS_JAVASCRIPT_FILE)
            .withXqueryFileOptionName(Options.READ_EXPAND_URIS_XQUERY_FILE)
            .withVarsPrefix(Options.READ_EXPAND_URIS_VARS_PREFIX)
            .build(context);
